...
while True:
    ...
    X3min = 10
    X3max = 15
    print("Variant 227 X1({0};{1}) X2({2};{3}) X3({4};{5})".format(X1min, X1max, X2min, X2max, X3min, X3max))
    ...
    Y_row2 = [y1[1], y2[1], y3[1]]  # другий рядок у
    Y_row3 = [y1[2], y2[2], y3[2]]  # ...
    Y_row4 = [y1[3], y2[3], y3[3]]
    Y_row5 = [y1[4], y2[4], y3[4]]
    Y_row6 = [y1[5], y2[5], y3[5]]
    Y_row7 = [y1[6], y2[6], y3[6]]
    Y_row8 = [y1[7], y2[7], y3[7]]  # восьмий рядок у
    Y_average1 = np.average(Y_row1)  # середній у першого рядка
    ...
    Y_average8 = np.average(Y_row8)  # середній у восьмого рядка
    Y_average = [round(Y_average1, 3), round(Y_average2, 3), round(Y_average3, 3), round(Y_average4, 3),
                 # округлення до
                 round(Y_average5, 3), round(Y_average6, 3), round(Y_average7, 3), round(Y_average8, 3)]  # тисячних
    ...
    N = len(y1)
    F2 = N
    Gt = cohren_teor(F1, F2)  # теоретичне
    ...
    if cohren(Gp, Gt):
        ...
        beta_list = [0 for i in range(0,8)]
